...
/**
 * @note 修改：添加了 `FDB_USING_XF_FAL_MODE`.
 */
...
#if !defined(FDB_USING_FAL_MODE) && !defined(FDB_USING_FILE_MODE) && !defined(FDB_USING_XF_FAL_MODE)
#error "Please defined the FDB_USING_FAL_MODE or FDB_USING_FILE_MODE or FDB_USING_XF_FAL_MODE macro"
#endif
...
#elif defined(FDB_USING_XF_FAL_MODE)
        size_t block_size;

        /* FAL (Flash Abstraction Layer) initialization */
        xf_fal_init();
        /* check the flash partition */
        if ((db->storage.part = xf_fal_partition_find(path)) == NULL) {
            FDB_INFO("Error: Partition (%s) not found.\n", path);
            return FDB_PART_NOT_FOUND;
        }

        block_size = xf_fal_flash_device_find(db->storage.part->flash_name)->sector_size;
        if (db->sec_size == 0) {
            db->sec_size = block_size;
        } else {
            /* must be aligned with block size */
