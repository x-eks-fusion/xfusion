...
fdb_err_t _fdb_flash_read(fdb_db_t db, uint32_t addr, void *buf, size_t size)
{
    ...
    if (db->file_mode) {
        ...
        if (xf_fal_partition_read(db->storage.part, addr, (uint8_t *) buf, size) != XF_OK) {
            result = FDB_READ_ERR;
        }
#endif
    }
...
#elif defined(FDB_USING_XF_FAL_MODE)
        if (xf_fal_partition_erase(db->storage.part, addr, size) != XF_OK) {
            result = FDB_ERASE_ERR;
        }
#endif
    }
...
#elif defined(FDB_USING_XF_FAL_MODE)
        if (xf_fal_partition_write(db->storage.part, addr, (uint8_t *) buf, size) != XF_OK) {
            result = FDB_WRITE_ERR;
        }
#endif
    }
