/**
 * @example examples/wireless/ble/hid_mouse_server/main/xf_main.c
 * xf_wal ble gatt HID 服务端 示例。
 */
...
const static uint16_t s_svc_uuid16_list[] = {BLE_UUID_HUMAN_INTERFACE_DEVICE};
    ...
    {
        {
            ...
            .ad_data.flag = (XF_BLE_FLAGS_LE_GENERAL | XF_BLE_FLAGS_BR_EDR_NOT_SUPPORTED),
            .is_ptr = false,
        }, 
        {
        ...
        {
            .ad_type = XF_BLE_ADV_STRUCT_TYPE_SVC_UUID16_LIST_ALL,
            .ad_data_len = sizeof(s_svc_uuid16_list),
            .ad_data.uuid_list = (uint8_t *)s_svc_uuid16_list,
            .is_ptr = true,
        },
        {0}
    },
...
void xf_main(void)
{
    ...
    XF_LOGI(TAG, "SVC HDL %d", s_svc_set.handle);

    s_value_handle = s_svc_set.chara_set[HID_CHARA_INDEX_INPUT_REPORT].value_handle;
    // 启动服务
    ...
    xf_ble_gap_evt_cb_param_t *param)
{
    ...
    case XF_BLE_GAP_EVT_PAIR_REQ: {
        ...
        xf_ble_gap_set_pair_feature(param->pair_req.conn_id, 
            XF_BLE_SM_PAIR_FEATURE_IO_CAP, XF_BLE_SM_IO_CAP_NONE);
        xf_ble_gap_set_pair_feature(param->pair_req.conn_id, 
            XF_BLE_SM_PAIR_FEATURE_AUTHEN_RQMT, XF_BLE_SM_AUTHEN_RQMT_BOND);
        xf_ble_gap_respond_pair(param->pair_req.conn_id);
    } break;
    case XF_BLE_GAP_EVT_PAIR_PASSKEY_REQ: {
        uint32_t pin_code = 123456;
        xf_ble_gap_pair_exchange_passkey(param->pair_req.conn_id, pin_code);
        XF_LOGI(TAG, "EV:pair_passkey_req:pin_code:%d", pin_code);
    }break;
    /* 事件: 断连  */
    ...
    xf_ble_gatts_evt_cb_param_t *param)
{
    ...
    case XF_BLE_GATTS_EVT_READ_REQ: {
        ...
        XF_LOGI(TAG, ">>>> HDL:%d,%d,%d,%d,%d", 
            ...
            s_chara_set[HID_CHARA_INDEX_INPUT_REPORT].value_handle,
            s_chara_set[HID_CHARA_INDEX_CTRL_POINT].value_handle);
